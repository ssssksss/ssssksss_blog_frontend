import type { Config } from "tailwindcss";
import { PluginAPI } from "tailwindcss/types/config";

export function extractFontFamilies(
  cssContent: string,
): Record<string, string> {
  const regex = /font-family:\s*["']?(.*?)["']?\s*;/g;
  const fontFamilies: Record<string, string> = {};
  let match: RegExpExecArray | null;

  while ((match = regex.exec(cssContent)) !== null) {
    const font = match[1].trim();
    fontFamilies[font] = font; // 키와 값을 동일하게 설정
  }

  return fontFamilies;
}

const fonts = extractFontFamilies(`
  font-family: "gmarketSansBold";
  font-family: "cyarn ookieRunRegular";
  font-family: "typoHelloPOP";
  font-family: "yanoljaYacheBold";
  font-family: "yanoljaYacheRegular";
  font-family: "D2Coding";
  font-family: "bitbit";
  font-family: "SDSamliphopangche_Outline";
  font-family: "Ownglyph_meetme-Rg";
  font-family: "DNFBitBitv2";
  font-family: "HakgyoansimPuzzleTTF-Black";
  font-family: "WavvePADO-Regular";
  font-family: "OKCHAN";
`);

const config: Config = {
  mode: "jit",
  darkMode: "class",
  content: [
    "./src/component/**/view/**/*.{ts,tsx}",
    "./src/component/**/hybrid/**/*.{ts,tsx}",
    "./src/component/common/**/*.{ts,tsx}",
    "./src/app/**/*.{ts,tsx}",
    "./src/app/*.{ts,tsx}",
    "./src/utils/**/*.{ts,tsx}",
  ],
  safelist: [
    {
      pattern:
        /^text-(red|orange|yellow|green|skyblue|blue|purple|pink|black|gray|white)-contrast$/,
    },
  ],
  theme: {
    extend: {
      fontFamily: {
        ...fonts, // 동적으로 가져온 폰트 추가
      },
      height: {
        "btn-sm": "40px", // small
        "btn-md": "44px", // medium
        "btn-lg": "48px", // large
        "btn-xl": "56px", // extra large
      },
      screens: {
        sm: "640px",
        md: "768px",
        lg: "1024px",
        xl: "1280px",
        "2xl": "1536px",
      },
      borderRadius: {
        primary: "1rem",
      },
      colors: {
        default: {
          1: "var(--bg-default-1)",
          2: "var(--bg-default-2)",
        },
        contrast: {
          1: "var(--bg-text-contrast-1)",
          2: "var(--bg-text-contrast-2)",
        },
        primary: {
          100: "var(--primary-color-100)",
          90: "var(--primary-color-90)",
          80: "var(--primary-color-80)",
          70: "var(--primary-color-70)",
          60: "var(--primary-color-60)",
          50: "var(--primary-color-50)",
          40: "var(--primary-color-40)",
          30: "var(--primary-color-30)",
          20: "var(--primary-color-20)",
          10: "var(--primary-color-10)",
          contrast: "var(--primary-color-contrast)",
        },
        secondary: {
          100: "var(--secondary-color-100)",
          90: "var(--secondary-color-90)",
          80: "var(--secondary-color-80)",
          70: "var(--secondary-color-70)",
          60: "var(--secondary-color-60)",
          50: "var(--secondary-color-50)",
          40: "var(--secondary-color-40)",
          30: "var(--secondary-color-30)",
          20: "var(--secondary-color-20)",
          10: "var(--secondary-color-10)",
          contrast: "var(--secondary-color-contrast)",
        },
        third: {
          100: "var(--third-color-100)",
          90: "var(--third-color-90)",
          80: "var(--third-color-80)",
          70: "var(--third-color-70)",
          60: "var(--third-color-60)",
          50: "var(--third-color-50)",
          40: "var(--third-color-40)",
          30: "var(--third-color-30)",
          20: "var(--third-color-20)",
          10: "var(--third-color-10)",
          contrast: "var(--third-color-contrast)",
        },
        red: {
          100: "#FF0000",
          90: "#FF1717",
          80: "#FF2D2D",
          70: "#FF4444",
          60: "#FF5B5B",
          50: "#FF7171",
          40: "#FF8888",
          30: "#FF9F9F",
          20: "#FFB5B5",
          10: "#FFCCCC",
          contrast: "#F0F0F0",
          "contrast-40": "#666666",
          "contrast-30": "#4C4C4C",
          "contrast-20": "#333333",
          "contrast-10": "#191919",
        },
        orange: {
          orange: {
            100: "#FF9900",
            90: "#FFA31A",
            80: "#FFAD33",
            70: "#FFB84D",
            60: "#FFC266",
            50: "#FFCC80",
            40: "#FFD699",
            30: "#FFE0B3",
            20: "#FFEBCC",
            10: "#FFF5E6",
            contrast: "#FFFFFF",
            "contrast-40": "#666666",
            "contrast-30": "#4C4C4C",
            "contrast-20": "#333333",
            "contrast-10": "#191919",
          },
        },
        yellow: {
          100: "#FAFF00",
          90: "#FBFF1A",
          80: "#FBFF33",
          70: "#FCFF4D",
          60: "#FCFF66",
          50: "#FDFF80",
          40: "#FDFF99",
          30: "#FEFFB3",
          20: "#FEFFCC",
          10: "#FFFFE6",
          contrast: "#333333",
          "contrast-100": "#FFFFFF",
          "contrast-90": "#E5E5E5",
          "contrast-80": "#CCCCCC",
          "contrast-70": "#B2B2B2",
        },
        green: {
          100: "#18A934",
          90: "#32B44B",
          80: "#46BA5D",
          70: "#5AC06F",
          60: "#74CB85",
          50: "#8FD79B",
          40: "#A3DDAE",
          30: "#B7E3C1",
          20: "#D1EED6",
          10: "#EAF8EC",
          contrast: "#333333",
          "contrast-40": "#666666",
          "contrast-30": "#4C4C4C",
          "contrast-20": "#333333",
          "contrast-10": "#191919",
        },
        skyblue: {
          100: "#56CCF2",
          90: "#67D2F3",
          80: "#78D6F5",
          70: "#89DBF6",
          60: "#9AE0F7",
          50: "#ABE5F9",
          40: "#BBEBFA",
          30: "#CCF0FB",
          20: "#DDF5FC",
          10: "#EEF9FE",
          contrast: "#FFFFFF",
          "contrast-40": "#666666",
          "contrast-30": "#4C4C4C",
          "contrast-20": "#333333",
          "contrast-10": "#191919",
        },
        blue: {
          100: "#3454FF",
          90: "#4C67FF",
          80: "#5D76FF",
          70: "#6E84FF",
          60: "#8598FF",
          50: "#9BAAFF",
          40: "#AEBBFF",
          30: "#C2CCFF",
          20: "#D6DDFF",
          10: "#EAEEFF",
          contrast: "#FFFFFF",
          "contrast-40": "#666666",
          "contrast-30": "#4C4C4C",
          "contrast-20": "#333333",
          "contrast-10": "#191919",
        },
        purple: {
          100: "#9B51E0",
          90: "#A966E4",
          80: "#AF74E6",
          70: "#B782EA",
          60: "#C397EC",
          50: "#CFACF0",
          40: "#D7B9F3",
          30: "#E3CEF6",
          20: "#EBDCF9",
          10: "#F6F0FC",
          contrast: "#000000",
          "contrast-100": "#FFFFFF",
          "contrast-90": "#E5E5E5",
          "contrast-80": "#CCCCCC",
          "contrast-70": "#B2B2B2",
          "contrast-60": "#999999",
          "contrast-50": "#7F7F7F",
        },
        pink: {
          100: "#FFB8B8",
          90: "#FFC0C0",
          80: "#FFC6C6",
          70: "#FFCDCD",
          60: "#FFD4D4",
          50: "#FFDCDC",
          40: "#FFE3E3",
          30: "#FFEAEA",
          20: "#FFF1F1",
          10: "#FFF8F8",
          contrast: "#FFFFFF",
          "contrast-40": "#666666",
          "contrast-30": "#4C4C4C",
          "contrast-20": "#333333",
          "contrast-10": "#191919",
        },
        black: {
          100: "#000000", // 순수 블랙 (기본)
          90: "#0D0D0D", // 매우 진한 블랙
          80: "#1A1A1A", // 진한 블랙 (기존과 유사)
          70: "#262626", // 다크 그레이에 가까운 블랙
          60: "#333333", // 다크 그레이 (기존과 유사)
          50: "#4D4D4D", // 미디엄 다크 그레이
          40: "#666666", // 미디엄 그레이 (기존과 유사)
          30: "#808080", // 중간 그레이 (기존과 유사)
          20: "#999999", // 라이트 그레이 (기존과 유사)
          10: "#B3B3B3", // 연한 그레이
          contrast: "#F9FAFB", // 대비용 화이트
          "contrast-30": "#4C4C4C",
          "contrast-20": "#333333",
          "contrast-10": "#191919",
        },
        gray: {
          100: "#111111", // 가장 어두운 회색
          90: "#1F1F1F",
          80: "#2E2E2E",
          70: "#3D3D3D",
          60: "#5C5C5C",
          50: "#7A7A7A",
          40: "#A0A0A0",
          30: "#C6C6C6",
          20: "#E0E0E0",
          10: "#F5F5F5", // 가장 밝은 회색
          contrast: "#FFFFFF", // 일반 텍스트 대비용
          "contrast-40": "#4D4D4D", // gray 60쯤
          "contrast-30": "#333333", // gray 80쯤
          "contrast-20": "#1A1A1A", // gray 90쯤
          "contrast-10": "#000000", // 완전한 블랙
        },
        white: {
          100: "#FFFFFF",
          90: "#FEFEFE",
          80: "#FCFCFC",
          70: "#FAFAFA",
          60: "#F9F9F9",
          50: "#F7F7F7",
          40: "#F5F5F5",
          30: "#F3F3F3",
          20: "#F2F2F2",
          10: "#F0F0F0",
          contrast: "#333333",
          "contrast-100": "#FFFFFF",
          "contrast-90": "#E5E5E5",
          "contrast-80": "#CCCCCC",
          "contrast-70": "#B2B2B2",
        },
      },
      backgroundImage: {
        "gradient-purple-40-blue-40-70deg":
          "linear-gradient(70deg, #D7B9F3 0%, #AEBBFF 100%)",
      },
      boxShadow: {
        up: "0 -4px 6px -1px rgba(195, 151, 236, 0.4), 0 -2px 4px -1px rgba(195, 151, 236, 0.2), 0 2px 2px 1px rgba(195, 151, 236, 0.25)",
        "top-md":
          "0 -4px 6px -1px rgba(0, 0, 0, 0.1), 0 -2px 4px -1px rgba(0, 0, 0, 0.06)", // 중간 크기 위쪽 그림자
        "top-lg":
          "0 -10px 15px -3px rgba(0, 0, 0, 0.1), 0 -4px 6px -2px rgba(0, 0, 0, 0.05)", // 큰 크기 위쪽 그림자
      },
      keyframes: {
        marquee: {
          "0%": {transform: "translateX(0%)"},
          "100%": {transform: "translateX(-100%)"},
        },
        marquee5: {
          "0%": {transform: "translateX(0%)"},
          "50%": {transform: "translateX(-50%)"},
          "100%": {transform: "translateX(0%)"},
        },
        "fade-up": {
          "0%": {opacity: "0", transform: "translateY(100px)"},
          "100%": {opacity: "1", transform: "translateY(0)"},
        },
        marqueeContent: {
          "0%": {
            transform: "translateX(-100%)",
          },
          "40%": {transform: "translateX(0%)"},
          "60%": {
            transform: "translateX(0%)",
          },
          "100%": {transform: "translateX(-100%)"},
          // "0%": {
          //   transform:
          //     "translateX(calc(-100% + 3/7 * 1rem + 100/7 * 1vw - 1.5rem))",
          // },
          // "100%": {transform: "translateX(0%)"},
        },
        pulseSkeleton: {
          "0%": {
            "background-image":
              "linear-gradient(90deg, #9c9c9c 25%, #999999 50%, #9f9f9f 75%)",
          },
          "50%": {
            "background-image":
              "linear-gradient(90deg, #9f9f9f 25%, #9c9c9c 50%, #999999 75%)",
          },
          "100%": {
            "background-image":
              "linear-gradient(90deg, #999999 25%, #9f9f9f 50%, #9c9c9c 75%)",
          },
        },
        outlineBlink: {
          "0%, 100%": {
            outlineColor: "#C397EC",
          }, // 애니메이션 시작과 끝 색상
          "50%": {outlineColor: "transparent"}, // 중간에 투명하게
        },
        fadeIn: {
          "0%": {
            opacity: "0",
            transform: "scale(95%,95%)",
          },
          "100%": {
            opacity: "1",
            transform: "scale(100%,100%)",
          },
        },
        fill: {
          "0%": {
            backgroundPosition: "200% 0",
          },
          "100%": {
            backgroundPosition: "0 0",
          },
        },
        updown: {
          "0%, 100%": {transform: "translateY(4px)"}, // 처음과 끝에는 제자리
          "50%": {transform: "translateY(-4px)"}, // 중간에는 위로 10px 이동
        },
        rotateFadeIn: {
          "0%": {
            transform: "translateY(50%) rotate3d(1, 0, 0, 45deg)",
            opacity: "0.3",
          },
          "50%": {
            transform: "translateY(25%) rotate3d(1, 0, 0, 30deg)",
            opacity: "0.7",
          },
          "100%": {
            transform: "translateY(0%) rotate3d(1, 0, 0, 0deg)",
            opacity: "1",
          },
        },
        modalGrowingScale: {
          "0%": {
            transform: "scale(0%)",
          },
          "100%": {
            transform: "scale(100%)",
          },
        },
      },
      animation: {
        marquee: "marquee 10s linear infinite",
        marquee5: "marquee5 10s linear infinite alternate",
        marqueeContent: "marqueeContent 8s linear infinite alternate",
        pulseSkeleton: "pulseSkeleton 1s cubic-bezier(0.4, 0, 0.6, 1) infinite",
        slowSpin: "spin 12s linear infinite",
        fadeUp: "fade-up 1.5s ease-out",
        outlineBlink: "outlineBlink 2s infinite",
        fadeIn: "fadeIn 0.8s ease-in-out forwards",
        rotateFadeIn: "rotateFadeIn 0.2s linear",
        fill: "fill 4s linear infinite",
        updown: "updown 1.4s linear infinite",
        modalGrowingScale: "modalGrowingScale 0.3s linear",
      },
      willChange: {
        transform: "transform",
      },
      transformOrigin: {
        "center-center": "center center",
      },
    },
  },
  plugins: [
    require("tailwind-scrollbar-hide"),
    function ({addUtilities, theme, addBase}: PluginAPI) {
      // border, outline은 100
      // 배경은 40
      // 버튼색상 80
      addBase({
        "@media (prefers-color-scheme: dark)": {
          ":root": {
            "--primary-color-100": `${theme("colors.black.40")}`,
            "--primary-color-90": `${theme("colors.black.40")}`,
            "--primary-color-80": `${theme("colors.black.40")}`,
            "--primary-color-70": `${theme("colors.black.40")}`,
            "--primary-color-60": `${theme("colors.black.40")}`,
            "--primary-color-50": `${theme("colors.black.40")}`,
            "--primary-color-40": `${theme("colors.black.40")}`,
            "--primary-color-30": `${theme("colors.black.40")}`,
            "--primary-color-20": `${theme("colors.black.40")}`,
            "--primary-color-10": `${theme("colors.black.40")}`,
            "--primary-color-contrast": `${theme("colors.black.contrast")}`,
            "--secondary-color-100": `${theme("colors.black.40")}`,
            "--secondary-color-90": `${theme("colors.black.40")}`,
            "--secondary-color-80": `${theme("colors.black.40")}`,
            "--secondary-color-70": `${theme("colors.black.40")}`,
            "--secondary-color-60": `${theme("colors.black.40")}`,
            "--secondary-color-50": `${theme("colors.black.40")}`,
            "--secondary-color-40": `${theme("colors.black.40")}`,
            "--secondary-color-30": `${theme("colors.black.40")}`,
            "--secondary-color-20": `${theme("colors.black.40")}`,
            "--secondary-color-10": `${theme("colors.black.40")}`,
            "--secondary-color-contrast": `${theme("colors.black.contrast")}`,
            "--third-color-100": `${theme("colors.black.40")}`,
            "--third-color-90": `${theme("colors.black.40")}`,
            "--third-color-80": `${theme("colors.black.40")}`,
            "--third-color-70": `${theme("colors.black.40")}`,
            "--third-color-60": `${theme("colors.black.40")}`,
            "--third-color-50": `${theme("colors.black.40")}`,
            "--third-color-40": `${theme("colors.black.40")}`,
            "--third-color-30": `${theme("colors.black.40")}`,
            "--third-color-20": `${theme("colors.black.40")}`,
            "--third-color-10": `${theme("colors.black.40")}`,
            "--third-color-contrast": `${theme("colors.black.contrast")}`,
            "--bg-default-1": `${theme("colors.black.80")}`,
            "--bg-default-2": `${theme("colors.gray.40")}`,
            "--bg-disabled": `${theme("colors.black.60")}`,
            "--bg-text-contrast-1": `${theme("colors.white.80")}`,
            "--bg-text-contrast-2": `${theme("colors.black.80")}`,
            "--bg-text-disabled": `${theme("colors.white.20")}`,
            "--bg-glassmorphism": "rgba(222, 222, 222, 0.1)",
            "--dynamic-opacity": "0.9",
          },
        },
        "@media (prefers-color-scheme: light)": {
          ":root": {
            "--bg-default-1": `${theme("colors.gray.20")}`,
            "--bg-default-2": `${theme("colors.black.40")}`,
            "--bg-disabled": `${theme("colors.gray.60")}`,
            "--bg-text-contrast-1": `${theme("colors.black.80")}`,
            "--bg-text-contrast-2": `${theme("colors.white.80")}`,
            "--bg-text-disabled": `${theme("colors.black.80")}`,
            "--bg-glassmorphism": "rgba(255, 255, 255, 0.2)",
            "--dynamic-opacity": "0.8",
          },
          "[data-theme1=\"red\"]": {
            "--primary-color-100": `${theme("colors.red.100")}`,
            "--primary-color-90": `${theme("colors.red.90")}`,
            "--primary-color-80": `${theme("colors.red.80")}`,
            "--primary-color-70": `${theme("colors.red.70")}`,
            "--primary-color-60": `${theme("colors.red.60")}`,
            "--primary-color-50": `${theme("colors.red.50")}`,
            "--primary-color-40": `${theme("colors.red.40")}`,
            "--primary-color-30": `${theme("colors.red.30")}`,
            "--primary-color-20": `${theme("colors.red.20")}`,
            "--primary-color-10": `${theme("colors.red.10")}`,
            "--primary-color-contrast": `${theme("colors.red.contrast")}`,
          },
          "[data-theme1=\"orange\"]": {
            "--primary-color-100": `${theme("colors.orange.100")}`,
            "--primary-color-90": `${theme("colors.orange.90")}`,
            "--primary-color-80": `${theme("colors.orange.80")}`,
            "--primary-color-70": `${theme("colors.orange.70")}`,
            "--primary-color-60": `${theme("colors.orange.60")}`,
            "--primary-color-50": `${theme("colors.orange.50")}`,
            "--primary-color-40": `${theme("colors.orange.40")}`,
            "--primary-color-30": `${theme("colors.orange.30")}`,
            "--primary-color-20": `${theme("colors.orange.20")}`,
            "--primary-color-10": `${theme("colors.orange.10")}`,
            "--primary-color-contrast": `${theme("colors.orange.contrast")}`,
          },
          "[data-theme1=\"yellow\"]": {
            "--primary-color-100": `${theme("colors.yellow.100")}`,
            "--primary-color-90": `${theme("colors.yellow.90")}`,
            "--primary-color-80": `${theme("colors.yellow.80")}`,
            "--primary-color-70": `${theme("colors.yellow.70")}`,
            "--primary-color-60": `${theme("colors.yellow.60")}`,
            "--primary-color-50": `${theme("colors.yellow.50")}`,
            "--primary-color-40": `${theme("colors.yellow.40")}`,
            "--primary-color-30": `${theme("colors.yellow.30")}`,
            "--primary-color-20": `${theme("colors.yellow.20")}`,
            "--primary-color-10": `${theme("colors.yellow.10")}`,
            "--primary-color-contrast": `${theme("colors.yellow.contrast")}`,
          },
          "[data-theme1=\"green\"]": {
            "--primary-color-100": `${theme("colors.green.100")}`,
            "--primary-color-90": `${theme("colors.green.90")}`,
            "--primary-color-80": `${theme("colors.green.80")}`,
            "--primary-color-70": `${theme("colors.green.70")}`,
            "--primary-color-60": `${theme("colors.green.60")}`,
            "--primary-color-50": `${theme("colors.green.50")}`,
            "--primary-color-40": `${theme("colors.green.40")}`,
            "--primary-color-30": `${theme("colors.green.30")}`,
            "--primary-color-20": `${theme("colors.green.20")}`,
            "--primary-color-10": `${theme("colors.green.10")}`,
            "--primary-color-contrast": `${theme("colors.green.contrast")}`,
          },
          "[data-theme1=\"skyblue\"]": {
            "--primary-color-100": `${theme("colors.sky.100")}`,
            "--primary-color-90": `${theme("colors.sky.90")}`,
            "--primary-color-80": `${theme("colors.sky.80")}`,
            "--primary-color-70": `${theme("colors.sky.70")}`,
            "--primary-color-60": `${theme("colors.sky.60")}`,
            "--primary-color-50": `${theme("colors.sky.50")}`,
            "--primary-color-40": `${theme("colors.sky.40")}`,
            "--primary-color-30": `${theme("colors.sky.30")}`,
            "--primary-color-20": `${theme("colors.sky.20")}`,
            "--primary-color-10": `${theme("colors.sky.10")}`,
            "--primary-color-contrast": `${theme("colors.sky.contrast")}`,
          },
          "[data-theme1=\"blue\"]": {
            "--primary-color-100": `${theme("colors.blue.100")}`,
            "--primary-color-90": `${theme("colors.blue.90")}`,
            "--primary-color-80": `${theme("colors.blue.80")}`,
            "--primary-color-70": `${theme("colors.blue.70")}`,
            "--primary-color-60": `${theme("colors.blue.60")}`,
            "--primary-color-50": `${theme("colors.blue.50")}`,
            "--primary-color-40": `${theme("colors.blue.40")}`,
            "--primary-color-30": `${theme("colors.blue.30")}`,
            "--primary-color-20": `${theme("colors.blue.20")}`,
            "--primary-color-10": `${theme("colors.blue.10")}`,
            "--primary-color-contrast": `${theme("colors.blue.contrast")}`,
          },
          "[data-theme1=\"purple\"]": {
            "--primary-color-100": `${theme("colors.purple.100")}`,
            "--primary-color-90": `${theme("colors.purple.90")}`,
            "--primary-color-80": `${theme("colors.purple.80")}`,
            "--primary-color-70": `${theme("colors.purple.70")}`,
            "--primary-color-60": `${theme("colors.purple.60")}`,
            "--primary-color-50": `${theme("colors.purple.50")}`,
            "--primary-color-40": `${theme("colors.purple.40")}`,
            "--primary-color-30": `${theme("colors.purple.30")}`,
            "--primary-color-20": `${theme("colors.purple.20")}`,
            "--primary-color-10": `${theme("colors.purple.10")}`,
            "--primary-color-contrast": `${theme("colors.purple.contrast")}`,
          },
          "[data-theme1=\"pink\"]": {
            "--primary-color-100": `${theme("colors.pink.100")}`,
            "--primary-color-90": `${theme("colors.pink.90")}`,
            "--primary-color-80": `${theme("colors.pink.80")}`,
            "--primary-color-70": `${theme("colors.pink.70")}`,
            "--primary-color-60": `${theme("colors.pink.60")}`,
            "--primary-color-50": `${theme("colors.pink.50")}`,
            "--primary-color-40": `${theme("colors.pink.40")}`,
            "--primary-color-30": `${theme("colors.pink.30")}`,
            "--primary-color-20": `${theme("colors.pink.20")}`,
            "--primary-color-10": `${theme("colors.pink.10")}`,
            "--primary-color-contrast": `${theme("colors.pink.contrast")}`,
          },
          "[data-theme1=\"black\"]": {
            "--primary-color-100": `${theme("colors.black.100")}`,
            "--primary-color-90": `${theme("colors.black.90")}`,
            "--primary-color-80": `${theme("colors.black.80")}`,
            "--primary-color-70": `${theme("colors.black.70")}`,
            "--primary-color-60": `${theme("colors.black.60")}`,
            "--primary-color-50": `${theme("colors.black.50")}`,
            "--primary-color-40": `${theme("colors.black.40")}`,
            "--primary-color-30": `${theme("colors.black.30")}`,
            "--primary-color-20": `${theme("colors.black.20")}`,
            "--primary-color-10": `${theme("colors.black.10")}`,
            "--primary-color-contrast": `${theme("colors.black.contrast")}`,
          },
          "[data-theme1=\"gray\"]": {
            "--primary-color-100": `${theme("colors.gray.100")}`,
            "--primary-color-90": `${theme("colors.gray.90")}`,
            "--primary-color-80": `${theme("colors.gray.80")}`,
            "--primary-color-70": `${theme("colors.gray.70")}`,
            "--primary-color-60": `${theme("colors.gray.60")}`,
            "--primary-color-50": `${theme("colors.gray.50")}`,
            "--primary-color-40": `${theme("colors.gray.40")}`,
            "--primary-color-30": `${theme("colors.gray.30")}`,
            "--primary-color-20": `${theme("colors.gray.20")}`,
            "--primary-color-10": `${theme("colors.gray.10")}`,
            "--primary-color-contrast": `${theme("colors.gray.contrast")}`,
          },
          "[data-theme1=\"white\"]": {
            "--primary-color-100": `${theme("colors.white.100")}`,
            "--primary-color-90": `${theme("colors.white.90")}`,
            "--primary-color-80": `${theme("colors.white.80")}`,
            "--primary-color-70": `${theme("colors.white.70")}`,
            "--primary-color-60": `${theme("colors.white.60")}`,
            "--primary-color-50": `${theme("colors.white.50")}`,
            "--primary-color-40": `${theme("colors.white.40")}`,
            "--primary-color-30": `${theme("colors.white.30")}`,
            "--primary-color-20": `${theme("colors.white.20")}`,
            "--primary-color-10": `${theme("colors.white.10")}`,
            "--primary-color-contrast": `${theme("colors.white.contrast")}`,
          },
          "[data-theme2=\"red\"]": {
            "--secondary-color-100": `${theme("colors.red.100")}`,
            "--secondary-color-90": `${theme("colors.red.90")}`,
            "--secondary-color-80": `${theme("colors.red.80")}`,
            "--secondary-color-70": `${theme("colors.red.70")}`,
            "--secondary-color-60": `${theme("colors.red.60")}`,
            "--secondary-color-50": `${theme("colors.red.50")}`,
            "--secondary-color-40": `${theme("colors.red.40")}`,
            "--secondary-color-30": `${theme("colors.red.30")}`,
            "--secondary-color-20": `${theme("colors.red.20")}`,
            "--secondary-color-10": `${theme("colors.red.10")}`,
            "--secondary-color-contrast": `${theme("colors.red.contrast")}`,
          },
          "[data-theme2=\"orange\"]": {
            "--secondary-color-100": `${theme("colors.orange.100")}`,
            "--secondary-color-90": `${theme("colors.orange.90")}`,
            "--secondary-color-80": `${theme("colors.orange.80")}`,
            "--secondary-color-70": `${theme("colors.orange.70")}`,
            "--secondary-color-60": `${theme("colors.orange.60")}`,
            "--secondary-color-50": `${theme("colors.orange.50")}`,
            "--secondary-color-40": `${theme("colors.orange.40")}`,
            "--secondary-color-30": `${theme("colors.orange.30")}`,
            "--secondary-color-20": `${theme("colors.orange.20")}`,
            "--secondary-color-10": `${theme("colors.orange.10")}`,
            "--secondary-color-contrast": `${theme("colors.orange.contrast")}`,
          },
          "[data-theme2=\"yellow\"]": {
            "--secondary-color-100": `${theme("colors.yellow.100")}`,
            "--secondary-color-90": `${theme("colors.yellow.90")}`,
            "--secondary-color-80": `${theme("colors.yellow.80")}`,
            "--secondary-color-70": `${theme("colors.yellow.70")}`,
            "--secondary-color-60": `${theme("colors.yellow.60")}`,
            "--secondary-color-50": `${theme("colors.yellow.50")}`,
            "--secondary-color-40": `${theme("colors.yellow.40")}`,
            "--secondary-color-30": `${theme("colors.yellow.30")}`,
            "--secondary-color-20": `${theme("colors.yellow.20")}`,
            "--secondary-color-10": `${theme("colors.yellow.10")}`,
            "--secondary-color-contrast": `${theme("colors.yellow.contrast")}`,
          },
          "[data-theme2=\"green\"]": {
            "--secondary-color-100": `${theme("colors.green.100")}`,
            "--secondary-color-90": `${theme("colors.green.90")}`,
            "--secondary-color-80": `${theme("colors.green.80")}`,
            "--secondary-color-70": `${theme("colors.green.70")}`,
            "--secondary-color-60": `${theme("colors.green.60")}`,
            "--secondary-color-50": `${theme("colors.green.50")}`,
            "--secondary-color-40": `${theme("colors.green.40")}`,
            "--secondary-color-30": `${theme("colors.green.30")}`,
            "--secondary-color-20": `${theme("colors.green.20")}`,
            "--secondary-color-10": `${theme("colors.green.10")}`,
            "--secondary-color-contrast": `${theme("colors.green.contrast")}`,
          },
          "[data-theme2=\"skyblue\"]": {
            "--secondary-color-100": `${theme("colors.sky.100")}`,
            "--secondary-color-90": `${theme("colors.sky.90")}`,
            "--secondary-color-80": `${theme("colors.sky.80")}`,
            "--secondary-color-70": `${theme("colors.sky.70")}`,
            "--secondary-color-60": `${theme("colors.sky.60")}`,
            "--secondary-color-50": `${theme("colors.sky.50")}`,
            "--secondary-color-40": `${theme("colors.sky.40")}`,
            "--secondary-color-30": `${theme("colors.sky.30")}`,
            "--secondary-color-20": `${theme("colors.sky.20")}`,
            "--secondary-color-10": `${theme("colors.sky.10")}`,
            "--secondary-color-contrast": `${theme("colors.sky.contrast")}`,
          },
          "[data-theme2=\"blue\"]": {
            "--secondary-color-100": `${theme("colors.blue.100")}`,
            "--secondary-color-90": `${theme("colors.blue.90")}`,
            "--secondary-color-80": `${theme("colors.blue.80")}`,
            "--secondary-color-70": `${theme("colors.blue.70")}`,
            "--secondary-color-60": `${theme("colors.blue.60")}`,
            "--secondary-color-50": `${theme("colors.blue.50")}`,
            "--secondary-color-40": `${theme("colors.blue.40")}`,
            "--secondary-color-30": `${theme("colors.blue.30")}`,
            "--secondary-color-20": `${theme("colors.blue.20")}`,
            "--secondary-color-10": `${theme("colors.blue.10")}`,
            "--secondary-color-contrast": `${theme("colors.blue.contrast")}`,
          },
          "[data-theme2=\"purple\"]": {
            "--secondary-color-100": `${theme("colors.purple.100")}`,
            "--secondary-color-90": `${theme("colors.purple.90")}`,
            "--secondary-color-80": `${theme("colors.purple.80")}`,
            "--secondary-color-70": `${theme("colors.purple.70")}`,
            "--secondary-color-60": `${theme("colors.purple.60")}`,
            "--secondary-color-50": `${theme("colors.purple.50")}`,
            "--secondary-color-40": `${theme("colors.purple.40")}`,
            "--secondary-color-30": `${theme("colors.purple.30")}`,
            "--secondary-color-20": `${theme("colors.purple.20")}`,
            "--secondary-color-10": `${theme("colors.purple.10")}`,
            "--secondary-color-contrast": `${theme("colors.purple.contrast")}`,
          },
          "[data-theme2=\"pink\"]": {
            "--secondary-color-100": `${theme("colors.pink.100")}`,
            "--secondary-color-90": `${theme("colors.pink.90")}`,
            "--secondary-color-80": `${theme("colors.pink.80")}`,
            "--secondary-color-70": `${theme("colors.pink.70")}`,
            "--secondary-color-60": `${theme("colors.pink.60")}`,
            "--secondary-color-50": `${theme("colors.pink.50")}`,
            "--secondary-color-40": `${theme("colors.pink.40")}`,
            "--secondary-color-30": `${theme("colors.pink.30")}`,
            "--secondary-color-20": `${theme("colors.pink.20")}`,
            "--secondary-color-10": `${theme("colors.pink.10")}`,
            "--secondary-color-contrast": `${theme("colors.pink.contrast")}`,
          },
          "[data-theme2=\"black\"]": {
            "--secondary-color-100": `${theme("colors.black.100")}`,
            "--secondary-color-90": `${theme("colors.black.90")}`,
            "--secondary-color-80": `${theme("colors.black.80")}`,
            "--secondary-color-70": `${theme("colors.black.70")}`,
            "--secondary-color-60": `${theme("colors.black.60")}`,
            "--secondary-color-50": `${theme("colors.black.50")}`,
            "--secondary-color-40": `${theme("colors.black.40")}`,
            "--secondary-color-30": `${theme("colors.black.30")}`,
            "--secondary-color-20": `${theme("colors.black.20")}`,
            "--secondary-color-10": `${theme("colors.black.10")}`,
            "--secondary-color-contrast": `${theme("colors.black.contrast")}`,
          },
          "[data-theme2=\"gray\"]": {
            "--secondary-color-100": `${theme("colors.gray.100")}`,
            "--secondary-color-90": `${theme("colors.gray.90")}`,
            "--secondary-color-80": `${theme("colors.gray.80")}`,
            "--secondary-color-70": `${theme("colors.gray.70")}`,
            "--secondary-color-60": `${theme("colors.gray.60")}`,
            "--secondary-color-50": `${theme("colors.gray.50")}`,
            "--secondary-color-40": `${theme("colors.gray.40")}`,
            "--secondary-color-30": `${theme("colors.gray.30")}`,
            "--secondary-color-20": `${theme("colors.gray.20")}`,
            "--secondary-color-10": `${theme("colors.gray.10")}`,
            "--secondary-color-contrast": `${theme("colors.gray.contrast")}`,
          },
          "[data-theme2=\"white\"]": {
            "--secondary-color-100": `${theme("colors.white.100")}`,
            "--secondary-color-90": `${theme("colors.white.90")}`,
            "--secondary-color-80": `${theme("colors.white.80")}`,
            "--secondary-color-70": `${theme("colors.white.70")}`,
            "--secondary-color-60": `${theme("colors.white.60")}`,
            "--secondary-color-50": `${theme("colors.white.50")}`,
            "--secondary-color-40": `${theme("colors.white.40")}`,
            "--secondary-color-30": `${theme("colors.white.30")}`,
            "--secondary-color-20": `${theme("colors.white.20")}`,
            "--secondary-color-10": `${theme("colors.white.10")}`,
            "--secondary-color-contrast": `${theme("colors.white.contrast")}`,
          },

          "[data-theme3=\"red\"]": {
            "--third-color-100": `${theme("colors.red.100")}`,
            "--third-color-90": `${theme("colors.red.90")}`,
            "--third-color-80": `${theme("colors.red.80")}`,
            "--third-color-70": `${theme("colors.red.70")}`,
            "--third-color-60": `${theme("colors.red.60")}`,
            "--third-color-50": `${theme("colors.red.50")}`,
            "--third-color-40": `${theme("colors.red.40")}`,
            "--third-color-30": `${theme("colors.red.30")}`,
            "--third-color-20": `${theme("colors.red.20")}`,
            "--third-color-10": `${theme("colors.red.10")}`,
            "--third-color-contrast": `${theme("colors.red.contrast")}`,
          },
          "[data-theme3=\"orange\"]": {
            "--third-color-100": `${theme("colors.orange.100")}`,
            "--third-color-90": `${theme("colors.orange.90")}`,
            "--third-color-80": `${theme("colors.orange.80")}`,
            "--third-color-70": `${theme("colors.orange.70")}`,
            "--third-color-60": `${theme("colors.orange.60")}`,
            "--third-color-50": `${theme("colors.orange.50")}`,
            "--third-color-40": `${theme("colors.orange.40")}`,
            "--third-color-30": `${theme("colors.orange.30")}`,
            "--third-color-20": `${theme("colors.orange.20")}`,
            "--third-color-10": `${theme("colors.orange.10")}`,
            "--third-color-contrast": `${theme("colors.orange.contrast")}`,
          },
          "[data-theme3=\"yellow\"]": {
            "--third-color-100": `${theme("colors.yellow.100")}`,
            "--third-color-90": `${theme("colors.yellow.90")}`,
            "--third-color-80": `${theme("colors.yellow.80")}`,
            "--third-color-70": `${theme("colors.yellow.70")}`,
            "--third-color-60": `${theme("colors.yellow.60")}`,
            "--third-color-50": `${theme("colors.yellow.50")}`,
            "--third-color-40": `${theme("colors.yellow.40")}`,
            "--third-color-30": `${theme("colors.yellow.30")}`,
            "--third-color-20": `${theme("colors.yellow.20")}`,
            "--third-color-10": `${theme("colors.yellow.10")}`,
            "--third-color-contrast": `${theme("colors.yellow.contrast")}`,
          },
          "[data-theme3=\"green\"]": {
            "--third-color-100": `${theme("colors.green.100")}`,
            "--third-color-90": `${theme("colors.green.90")}`,
            "--third-color-80": `${theme("colors.green.80")}`,
            "--third-color-70": `${theme("colors.green.70")}`,
            "--third-color-60": `${theme("colors.green.60")}`,
            "--third-color-50": `${theme("colors.green.50")}`,
            "--third-color-40": `${theme("colors.green.40")}`,
            "--third-color-30": `${theme("colors.green.30")}`,
            "--third-color-20": `${theme("colors.green.20")}`,
            "--third-color-10": `${theme("colors.green.10")}`,
            "--third-color-contrast": `${theme("colors.green.contrast")}`,
          },
          "[data-theme3=\"skyblue\"]": {
            "--third-color-100": `${theme("colors.sky.100")}`,
            "--third-color-90": `${theme("colors.sky.90")}`,
            "--third-color-80": `${theme("colors.sky.80")}`,
            "--third-color-70": `${theme("colors.sky.70")}`,
            "--third-color-60": `${theme("colors.sky.60")}`,
            "--third-color-50": `${theme("colors.sky.50")}`,
            "--third-color-40": `${theme("colors.sky.40")}`,
            "--third-color-30": `${theme("colors.sky.30")}`,
            "--third-color-20": `${theme("colors.sky.20")}`,
            "--third-color-10": `${theme("colors.sky.10")}`,
            "--third-color-contrast": `${theme("colors.sky.contrast")}`,
          },
          "[data-theme3=\"blue\"]": {
            "--third-color-100": `${theme("colors.blue.100")}`,
            "--third-color-90": `${theme("colors.blue.90")}`,
            "--third-color-80": `${theme("colors.blue.80")}`,
            "--third-color-70": `${theme("colors.blue.70")}`,
            "--third-color-60": `${theme("colors.blue.60")}`,
            "--third-color-50": `${theme("colors.blue.50")}`,
            "--third-color-40": `${theme("colors.blue.40")}`,
            "--third-color-30": `${theme("colors.blue.30")}`,
            "--third-color-20": `${theme("colors.blue.20")}`,
            "--third-color-10": `${theme("colors.blue.10")}`,
            "--third-color-contrast": `${theme("colors.blue.contrast")}`,
          },
          "[data-theme3=\"purple\"]": {
            "--third-color-100": `${theme("colors.purple.100")}`,
            "--third-color-90": `${theme("colors.purple.90")}`,
            "--third-color-80": `${theme("colors.purple.80")}`,
            "--third-color-70": `${theme("colors.purple.70")}`,
            "--third-color-60": `${theme("colors.purple.60")}`,
            "--third-color-50": `${theme("colors.purple.50")}`,
            "--third-color-40": `${theme("colors.purple.40")}`,
            "--third-color-30": `${theme("colors.purple.30")}`,
            "--third-color-20": `${theme("colors.purple.20")}`,
            "--third-color-10": `${theme("colors.purple.10")}`,
            "--third-color-contrast": `${theme("colors.purple.contrast")}`,
          },
          "[data-theme3=\"pink\"]": {
            "--third-color-100": `${theme("colors.pink.100")}`,
            "--third-color-90": `${theme("colors.pink.90")}`,
            "--third-color-80": `${theme("colors.pink.80")}`,
            "--third-color-70": `${theme("colors.pink.70")}`,
            "--third-color-60": `${theme("colors.pink.60")}`,
            "--third-color-50": `${theme("colors.pink.50")}`,
            "--third-color-40": `${theme("colors.pink.40")}`,
            "--third-color-30": `${theme("colors.pink.30")}`,
            "--third-color-20": `${theme("colors.pink.20")}`,
            "--third-color-10": `${theme("colors.pink.10")}`,
            "--third-color-contrast": `${theme("colors.pink.contrast")}`,
          },
          "[data-theme3=\"black\"]": {
            "--third-color-100": `${theme("colors.gray.100")}`,
            "--third-color-90": `${theme("colors.gray.90")}`,
            "--third-color-80": `${theme("colors.gray.80")}`,
            "--third-color-70": `${theme("colors.gray.70")}`,
            "--third-color-60": `${theme("colors.gray.60")}`,
            "--third-color-50": `${theme("colors.gray.50")}`,
            "--third-color-40": `${theme("colors.gray.40")}`,
            "--third-color-30": `${theme("colors.gray.30")}`,
            "--third-color-20": `${theme("colors.gray.20")}`,
            "--third-color-10": `${theme("colors.gray.10")}`,
            "--third-color-contrast": `${theme("colors.gray.contrast")}`,
          },
          "[data-theme3=\"gray\"]": {
            "--third-color-100": `${theme("colors.gray.100")}`,
            "--third-color-90": `${theme("colors.gray.90")}`,
            "--third-color-80": `${theme("colors.gray.80")}`,
            "--third-color-70": `${theme("colors.gray.70")}`,
            "--third-color-60": `${theme("colors.gray.60")}`,
            "--third-color-50": `${theme("colors.gray.50")}`,
            "--third-color-40": `${theme("colors.gray.40")}`,
            "--third-color-30": `${theme("colors.gray.30")}`,
            "--third-color-20": `${theme("colors.gray.20")}`,
            "--third-color-10": `${theme("colors.gray.10")}`,
            "--third-color-contrast": `${theme("colors.gray.contrast")}`,
          },
          "[data-theme3=\"white\"]": {
            "--third-color-100": `${theme("colors.white.100")}`,
            "--third-color-90": `${theme("colors.white.90")}`,
            "--third-color-80": `${theme("colors.white.80")}`,
            "--third-color-70": `${theme("colors.white.70")}`,
            "--third-color-60": `${theme("colors.white.60")}`,
            "--third-color-50": `${theme("colors.white.50")}`,
            "--third-color-40": `${theme("colors.white.40")}`,
            "--third-color-30": `${theme("colors.white.30")}`,
            "--third-color-20": `${theme("colors.white.20")}`,
            "--third-color-10": `${theme("colors.white.10")}`,
            "--third-color-contrast": `${theme("colors.white.contrast")}`,
          },
          ".dark": {
            "--primary-color-100": `${theme("colors.black.40")}`,
            "--primary-color-90": `${theme("colors.black.40")}`,
            "--primary-color-80": `${theme("colors.black.40")}`,
            "--primary-color-70": `${theme("colors.black.40")}`,
            "--primary-color-60": `${theme("colors.black.40")}`,
            "--primary-color-50": `${theme("colors.black.40")}`,
            "--primary-color-40": `${theme("colors.black.40")}`,
            "--primary-color-30": `${theme("colors.black.40")}`,
            "--primary-color-20": `${theme("colors.black.40")}`,
            "--primary-color-10": `${theme("colors.black.40")}`,
            "--primary-color-contrast": `${theme("colors.black.contrast")}`,
            "--secondary-color-100": `${theme("colors.black.40")}`,
            "--secondary-color-90": `${theme("colors.black.40")}`,
            "--secondary-color-80": `${theme("colors.black.40")}`,
            "--secondary-color-70": `${theme("colors.black.40")}`,
            "--secondary-color-60": `${theme("colors.black.40")}`,
            "--secondary-color-50": `${theme("colors.black.40")}`,
            "--secondary-color-40": `${theme("colors.black.40")}`,
            "--secondary-color-30": `${theme("colors.black.40")}`,
            "--secondary-color-20": `${theme("colors.black.40")}`,
            "--secondary-color-10": `${theme("colors.black.40")}`,
            "--secondary-color-contrast": `${theme("colors.black.contrast")}`,
            "--third-color-100": `${theme("colors.black.40")}`,
            "--third-color-90": `${theme("colors.black.40")}`,
            "--third-color-80": `${theme("colors.black.40")}`,
            "--third-color-70": `${theme("colors.black.40")}`,
            "--third-color-60": `${theme("colors.black.40")}`,
            "--third-color-50": `${theme("colors.black.40")}`,
            "--third-color-40": `${theme("colors.black.40")}`,
            "--third-color-30": `${theme("colors.black.40")}`,
            "--third-color-20": `${theme("colors.black.40")}`,
            "--third-color-10": `${theme("colors.black.40")}`,
            "--third-color-contrast": `${theme("colors.black.contrast")}`,
            "--bg-default-1": `${theme("colors.black.80")}`,
            "--bg-default-2": `${theme("colors.gray.40")}`,
            "--bg-disabled": `${theme("colors.black.60")}`,
            "--bg-text-contrast-1": `${theme("colors.white.80")}`,
            "--bg-text-contrast-2": `${theme("colors.black.80")}`,
            "--bg-text-disabled": `${theme("colors.white.20")}`,
            "--bg-glassmorphism": "rgba(222, 222, 222, 0.1)",
            "--dynamic-opacity": "0.9",
          },
        },
      }),
      addUtilities({
        // ".scrollbar-hide": {
        //   "-ms-overflow-style": "none" /* IE and Edge */,
        //   "scrollbar-width": "none" /* Firefox */,
        //   "&::-webkit-scrollbar": {
        //     display: "none" /* Chrome, Safari, and Opera */,
        //   },
        // },
        ".primary-border-radius": {
          "border-width": "0.0625rem", // 기존 두께
          "border-style": "solid",
          "border-color": "var(--primary-color-80)",
          "border-radius": "1rem",
        },
        ".secondary-border-radius": {
          "border-width": "0.0625rem", // 기존 두께
          "border-style": "solid",
          "border-color": "var(--secondary-color-80)",
          "border-radius": "1rem",
        },
        ".third-border-radius": {
          "border-width": "0.0625rem", // 기존 두께
          "border-style": "solid",
          "border-color": "var(--third-color-80)",
          "border-radius": "1rem",
        },
        ".primary-border": {
          border: "0.0625rem solid var(--primary-color-80)",
        },
        ".secondary-border": {
          border: "0.0625rem solid var(--secondary-color-80)",
        },
        ".third-border": {
          border: "0.0625rem solid var(--third-color-80)",
        },
        ".primary-set": {
          background: `${theme("colors.primary.80")}`,
          color: `${theme("colors.primary.contrast")}`,
          "outline-color": `${theme("colors.primary.contrast")}`,
        },
        ".secondary-set": {
          background: `${theme("colors.secondary.80")}`,
          color: `${theme("colors.secondary.contrast")}`,
          "outline-color": `${theme("colors.secondary.contrast")}`,
        },
        ".third-set": {
          background: `${theme("colors.third.80")}`,
          color: `${theme("colors.third.contrast")}`,
        },
        ".disabled-set": {
          background: "var(--bg-disabled)",
          color: "var(--bg-text-disabled)",
        },
        ".dynamic-opacity": {
          // opacity: "var(--dynamic-opacity)",
          "mix-blend-mode": "multiply",
        },
        ".default-outline-nocolor": {
          outline: "0.0625rem solid",
          "outline-offset": "-0.0625rem",
          "border-radius": "1rem",
        },
        ".default-flex": {
          display: "flex",
          "justify-content": "center",
          "align-items": "center",
        },
        ".text-gradient": {
          background: "linear-gradient(to right, #C193EC 0%, #7D91FF 100%)",
          "-webkit-background-clip": "text",
          "-webkit-text-fill-color": "transparent",
        },
        ".text-gradient1": {
          background: "linear-gradient(70deg, #f2709c 0%, #ff9472 100%)",
          "-webkit-background-clip": "text",
          "-webkit-text-fill-color": "transparent",
        },
        ".glassmorphism": {
          "backdrop-filter": "blur(24px) saturate(180%)",
          "-webkit-backdrop-filter": "blur(24px) saturate(180%)",
          "background-color": "var(--bg-glassmorphism)",
          "box-shadow": "0 4px 20px rgba(0, 0, 0, 0.1)",
          "will-change": "backdrop-filter, transform",
        },
        ".bg-gradient": {
          background: `linear-gradient(to right, ${theme("colors.primary.60")} 0%, ${theme("colors.secondary.60")} 100%)`,
        },
        ".animate-paused": {
          "animation-play-state": "paused",
        },
        ".animate--duration-1": {
          "animate--duration": "1s",
        },
        ".fillAnimation": {
          background:
              "linear-gradient(70deg, #C193EC 0%, #7D91FF 50%, #C193EC 100%)",
          backgroundSize: "200% 100%",
        },
      });
    },
  ],
};
export default config;
